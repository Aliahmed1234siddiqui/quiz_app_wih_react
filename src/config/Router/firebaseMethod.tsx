import {
  ...
  export let fbAdd = ( body: any) => {
    return new Promise((resolve, reject) => {
      const taskId = push(ref(db, `question/`)).key;
      body.id = taskId;
      const reference = ref(db, `question/${body.id}`);
      set(reference, body)
        .then((res) => {
  ...
  export let fbAdd2 = ( body: any) => {
    return new Promise((resolve, reject) => {
      const taskId = push(ref(db, `quiz/`)).key;
      body.id = taskId;
      const reference = ref(db, `quiz/${body.id}`);
      set(reference, body)
        .then((res) => {
          resolve("Data Sent Successfully!");
        })
